...
import cmatest.ZeldaEvalFunction;
...
public class ZeldaLevelPlayer {
	...
	public static void main(String[] args) throws IOException {
		...
		ZeldaEvalFunction eval = new ZeldaEvalFunction();
		...
		if (args.length > 0) {
			...
			if(strLatentVector.subSequence(0, 2).equals("[[")) {
				...
				// Now create the Zelda level from the combined list representation
		...
		ZeldaProcess zeldaProcess = new ZeldaProcess();
		zeldaProcess.launchZelda(new String[0], true); // true means there is a human player       
		zeldaProcess.simulateOneLevel(level);
