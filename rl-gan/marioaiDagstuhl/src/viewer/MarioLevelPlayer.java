	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length > 0) {
			...
			if(strLatentVector.subSequence(0, 2).equals("[[")) {
				...
				level = eval.levelFromLatentVectorZelda(latentVector);
				byte[][] levelMap = level.map; 
				byte[][] levelTransposed = new byte[9][13];
        		for (int k = 0; k < 9; k++) {
        			for (int j = 0; j < 13; j++) {
        				levelTransposed[k][j] = levelMap[j][k];
        			}
				}
        		for (int j = 0; j < levelTransposed.length; j++) {
        			System.out.println(Arrays.toString(levelTransposed[j]));
				}
			}
		} else {
