...
public class EvaluatorZelda implements Runnable
{
    Thread thisThread = null;
    EvaluationOptionsZelda evaluationOptions;

    private List<EvaluationInfoZelda> evaluationSummary = new ArrayList<EvaluationInfoZelda>();

    private void evaluateServerMode()
    {
        Server server = new Server(evaluationOptions.getServerAgentPort(), EnvironmentZelda.numberOfObservationElements, EnvironmentZelda.numberOfButtons);
        evaluationOptions.setAgent(new ServerAgentZelda(server, evaluationOptions.isFastTCP()));
        ...
        while (server.isRunning())
        {
            ...
            if (resetData.startsWith("reset"))
            {
                ...
                ((ServerAgentZelda)evaluationOptions.getAgent()).setFastTCP(evaluationOptions.isFastTCP());
                init(evaluationOptions);
                // Simulate One Level
                EvaluationInfoZelda evaluationInfo;
...
//                LOGGER.VERBOSE_MODE VM = (evaluationInfo.zeldaStatus == Zelda.STATUS_WIN) ? LOGGER.VERBOSE_MODE.INFO : LOGGER.VERBOSE_MODE.ALL;
//                LOGGER.println("run finished with result : " + evaluationInfo, VM);
                ...
                Collections.sort(evaluationSummary, new evBasicFitnessComparatorZelda());
...
//                return evaluationSummary;
    ...
    public List<EvaluationInfoZelda> evaluate()
    {
...
//            LOGGER.VERBOSE_MODE VM = (evaluationInfo.zeldaStatus == Zelda.STATUS_WIN) ? LOGGER.VERBOSE_MODE.INFO : LOGGER.VERBOSE_MODE.ALL;
//            LOGGER.println("run  finished with result : " + evaluationInfo, VM);
//            continueCondition = !GlobalOptions.StopSimulationIfWin || !(evaluationInfo.zeldaStatus == Zelda.STATUS_WIN);
//        }
...
//    }
    ...
    public String exportToMatLabFile()
    {
        ...
        try {
            ...
            bw.newLine();
            bw.write("BasicFitness = [");
            for (EvaluationInfoZelda ev : evaluationSummary)
    ...
    public void reset()
    {
        evaluationSummary = new ArrayList<EvaluationInfoZelda>();
    }

    public EvaluatorZelda(EvaluationOptionsZelda evaluationOptions)
    {                      
        init(evaluationOptions);
    }
    ...
    public void init(EvaluationOptionsZelda evaluationOptions)
    {
        ToolsConfiguratorZelda.CreateZeldaComponentFrame(
                evaluationOptions);
...
class evBasicFitnessComparatorZelda implements Comparator
{
    public int compare(Object o, Object o1)
    {
        double ei1Fitness = ((EvaluationInfoZelda)(o)).computeBasicFitness();
        double ei2Fitness = ((EvaluationInfoZelda)(o1)).computeBasicFitness();
...
class evCoinsFitnessComparatorZelda implements Comparator
{
    public int compare(Object o, Object o1)
    {
        int ei1Fitness = ((EvaluationInfoZelda)(o)).numberOfGainedCoins;

        int ei2Fitness = ((EvaluationInfoZelda)(o1)).numberOfGainedCoins;
        if (ei1Fitness < ei2Fitness)
            return 1;
        else if (ei1Fitness > ei2Fitness)
            return -1;
...
class evDistanceFitnessComparatorZelda implements Comparator
{
    public int compare(Object o, Object o1)
    {
        double ei1Fitness = ((EvaluationInfoZelda)(o)).computeDistancePassed();
        double ei2Fitness = ((EvaluationInfoZelda)(o1)).computeDistancePassed();
        if (ei1Fitness < ei2Fitness)
            return 1;
        else if (ei1Fitness > ei2Fitness)
            return -1;
