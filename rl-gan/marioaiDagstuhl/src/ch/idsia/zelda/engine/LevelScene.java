...
import ch.idsia.zelda.engine.level.BgLevelGenerator;
import ch.idsia.zelda.engine.level.Level;
import ch.idsia.zelda.engine.level.LevelGenerator;
import ch.idsia.zelda.engine.level.SpriteTemplate;
import ch.idsia.zelda.engine.sprites.*;
...
public class LevelScene extends Scene implements SpriteContext, Cloneable
{
    ...
    private ZeldaComponent renderer;
    ...
    public LevelScene(GraphicsConfiguration graphicsConfiguration, ZeldaComponent renderer, long seed, int levelDifficulty, int type, int levelLength, int timeLimit)
    {
...
//                        return -12; // hard formation border. Pay attention!
    ...
    {
        byte[][] ret = new byte[EnvironmentZelda.HalfObsWidth*2][EnvironmentZelda.HalfObsHeight*2];
        //TODO: Move to constants 16
        int ZeldaXInMap = (int)zelda.x/16;
        int ZeldaYInMap = (int)zelda.y/16;

        for (int y = ZeldaYInMap - EnvironmentZelda.HalfObsHeight, obsX = 0; y < ZeldaYInMap + EnvironmentZelda.HalfObsHeight; y++, obsX++)
        {
            for (int x = ZeldaXInMap - EnvironmentZelda.HalfObsWidth, obsY = 0; x < ZeldaXInMap + EnvironmentZelda.HalfObsWidth; x++, obsY++)
            {
                if (x >=0 /*  && x <= level.xExit */ && y >= 0 && y < level.height)
                {
                    ...
                    ret[obsX][obsY] = 0;
//                if (x == ZeldaXInMap && y == ZeldaYInMap)
//                    ret[obsX][obsY] = zelda.kind;
            }
        }
        return ret;
    }
    ...
    public byte[][] enemiesObservation(int ZLevel)
    {
        ...
        for (int w = 0; w < ret.length; w++)
            for (int h = 0; h < ret[0].length; h++)
                ret[w][h] = 0;
//        ret[EnvironmentZelda.HalfObsWidth][EnvironmentZelda.HalfObsHeight] = zelda.kind;
        for (Sprite sprite : sprites)
        {
            if (sprite.kind == zelda.kind)
                continue;
            if (sprite.mapX >= 0 &&
                sprite.mapX > ZeldaXInMap - EnvironmentZelda.HalfObsWidth &&
                sprite.mapX < ZeldaXInMap + EnvironmentZelda.HalfObsWidth &&
                sprite.mapY >= 0 &&
                sprite.mapY > ZeldaYInMap - EnvironmentZelda.HalfObsHeight &&
                sprite.mapY < ZeldaYInMap + EnvironmentZelda.HalfObsHeight )
            {
                int obsX = sprite.mapY - ZeldaYInMap + EnvironmentZelda.HalfObsHeight;
                int obsY = sprite.mapX - ZeldaXInMap + EnvironmentZelda.HalfObsWidth;
...
//        System.out.println("ret = " + ret);
    ...
    public List<String> LevelSceneAroundZeldaASCII(boolean Enemies, boolean LevelMap,
...
//        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(os));//        bw.write("\nTotal world width = " + level.width);
        ...
        if (level != null && zelda != null)
        {
            ret.add("Total world width = " + level.width);
            ret.add("Total world height = " + level.height);
            ret.add("Physical Zelda Position (x,y): (" + zelda.x + "," + zelda.y + ")");
            ret.add("Zelda Observation Width " + EnvironmentZelda.HalfObsWidth*2);
            ret.add("Zelda Observation Height " + EnvironmentZelda.HalfObsHeight*2);
            ret.add("X Exit Position: " + level.xExit);
            int ZeldaXInMap = (int)zelda.x/16;
            int ZeldaYInMap = (int)zelda.y/16;
            ret.add("Calibrated Zelda Position (x,y): (" + ZeldaXInMap + "," + ZeldaYInMap + ")\n");
...
//                        if (x >=0 && x <= level.xExit)
            ...
            {
//                ret.add("~ZLevel: Z" + ZLevelScene + "===========\nAll objects: (LevelScene[x,y], Sprite[x,y])==/* Zelda ~> MM */=====\n");
//                for (int x = 0; x < levelScene.length; ++x)
//                {
//                    String tmpData = "";
//                    for (int y = 0; y < levelScene[0].length; ++y)
...
//                }
                ...
                ret.add("~ZLevelScene: Z" + ZLevelScene + " ZLevelEnemies: Z" + ZLevelEnemies + " ; Merged observation /* Zelda ~> #M.# */");
                for (int x = 0; x < levelScene.length; ++x)
                {
                    String tmpData = "";
                    for (int y = 0; y < levelScene[0].length; ++y)
            ...
            ret.add("~level or zelda is not available");
        return ret;
    }
...
//        level = LevelGenerator.createLevel(320, 15, levelSeed);
    ...
    public void tick()
    {
        ...
        /*      if (recorder != null)
         {
         recorder.addTick(zelda.getKeyMask());
         }
         
         if (replayer!=null)
         {
         zelda.setKeys(replayer.nextTick());
         }*/
    ...
    {
        int xCam = (int) (zelda.xOld + (zelda.x - zelda.xOld) * alpha) - 160;
        int yCam = (int) (zelda.yOld + (zelda.y - zelda.yOld) * alpha) - 120;

        if (GlobalOptions.ZeldaAlwaysInCenter)
        {
        }
        else
        {
        ...
        layer.renderExit0(g, tick, paused?0:alpha, zelda.winTime==0);

        g.translate(-xCam, -yCam);

        // TODO: Dump out of render!
        if (zelda.cheatKeys[Zelda.KEY_DUMP_CURRENT_WORLD])
        ...
        for (Sprite sprite : sprites)
        {
            ...
            if (zelda.cheatKeys[Zelda.KEY_DUMP_CURRENT_WORLD] && sprite.mapX >= 0 && sprite.mapX < level.observation.length &&
...
//        drawStringDropShadow(g, "MARIO: " + df.format(Zelda.lives), 0, 0, 7);
//        drawStringDropShadow(g, "#########", 0, 1, 7);


        drawStringDropShadow(g, "DIFFICULTY:   " + df.format(this.levelDifficulty), 0, 0, this.levelDifficulty > 6 ? 1 : this.levelDifficulty > 2 ? 4 : 7 ); drawStringDropShadow(g, "CREATURES:" + (zelda.world.paused ? "OFF" : "ON"), 19, 0, 7);
        ...
        drawStringDropShadow(g, "LENGTH:" + (int)zelda.x/16 + " of " + this.levelLength, 0, 3, 7); drawStringDropShadow(g, "by Fire  : " + killedCreaturesByFireBall, 19, 2, 1);
        drawStringDropShadow(g,"COINS    : " + df.format(zelda.coins), 0, 4, 4);                      drawStringDropShadow(g, "by Shell : " + killedCreaturesByShell, 19, 3, 1);
        ...
        if (GlobalOptions.Labels)
        {
            ...
            g.drawString("x : " + zelda.x + "y: " + zelda.y, 70, 50);
            g.drawString("xOld : " + zelda.xOld + "yOld: " + zelda.yOld, 70, 60);
        }
...
//        zelda.x>level.xExit*16
        if (zelda.winTime > 0)
        {
            float t = zelda.winTime + alpha;
            t = t * t * 0.2f;
...
//                init();
            ...
            renderBlackout(g, zelda.xDeathPos - xCam, zelda.yDeathPos - yCam, (int) (320 - t));
        }

        if (zelda.deathTime > 0)
        {
//            float t = zelda.deathTime + alpha;
//            t = t * t * 0.4f;
//
//            if (t > 1800)
//            {
...
//            renderBlackout(g, (int) (zelda.xDeathPos - xCam), (int) (zelda.yDeathPos - yCam), (int) (320 - t));
        }
    }
    ...
    private void drawProgress(Graphics g) {
        ...
        int progressInChars = (int) (zelda.x * (entirePathStr.length()/physLength));
        String progress_str = "";
...
//    {
//        System.arraycopy(action, 0, zelda.keys, 0, 6);
//    }
    ...
    @Override protected Object clone() throws CloneNotSupportedException 
    {
    	...
    	c.zelda = (Zelda) this.zelda.clone();
