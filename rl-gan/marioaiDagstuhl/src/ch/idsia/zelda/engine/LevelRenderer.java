...
import java.awt.*;
import ch.idsia.zelda.engine.level.*;
import viewer.ZeldaLevelViewer;
...
public class LevelRenderer
{
	...
	public static void renderArea(Graphics2D g, Level level, int x0, int y0, int xCam, int yCam, int w, int h) {
		...
		ZeldaComponent mc = new ZeldaComponent(w,h);
		...
		for (int x = xTileStart; x <= xTileEnd; x++)
		{
			for (int y = yTileStart; y <= yTileEnd; y++)
			{
				...
				SpriteTemplate sprite = level.getSpriteTemplate((xCam/ZeldaLevelViewer.BLOCK_SIZE) + x,y); // Adding xCam so enemies render at the right location when level view is clipped
				if(sprite != null) {
	...
	public static void renderAreaWhiteBack(Graphics2D g, Level level, int x0, int y0, int xCam, int yCam, int w, int h) {
		...
		g.setBackground(Color.WHITE); // Might change this to add an interesting background
		g.clearRect(x0, y0, w, h);
		int xTileStart = (x0 + xCam) / 16;
		int yTileStart = (y0 + yCam) / 16;
		int xTileEnd = (x0 + xCam + w) / 16;
