...
import ch.idsia.tools.EvaluationInfoMario;
...
public class MarioEvalFunction implements IObjectiveFunction {
	 ...
	 * @return Array of those levels
	 */
	public static Level[] zeldaLevelsFromJson(String json) {
		List<List<List<Integer>>> allLevels = JsonReader.JsonToInt(json);
		Level[] result = new Level[allLevels.size()];
		int index = 0;
		for(List<List<Integer>> listRepresentation : allLevels) {
			result[index++] = LevelParser.createZeldaLevelJson(listRepresentation);
		}
		return result;
	}
    ...
    public void exit() throws IOException{
        ...
        String response = "";
		while(!response.equals("CLOSED")) {
			response = ganProcess.commRecv();
		}
		System.out.println("Python process shut down");
    }
	...
	/**
	 * Helper method to get the Zelda Level from the latent vector
	 * @param x Latent vector
	 * @return Zelda Level
	 * @throws IOException Problems communicating with Python GAN process
	 */
	public Level levelFromLatentVectorZelda(double[] x) throws IOException {
		x = mapArrayToOne(x);
		// Interpret x to a level
		// Brackets required since generator.py expects of list of multiple levels, though only one is being sent here
		ganProcess.commSend("[" + Arrays.toString(x) + "]");
		String levelString = ganProcess.commRecv(); // Response to command just sent
		Level[] levels = zeldaLevelsFromJson("[" +levelString + "]"); // Really only one level in this array
		Level level = levels[0];
		return level;
	}
	...
	public double valueOf(double[] x) {
		try {
			...
			// Do a simulation
			EvaluationInfoMario info = this.marioProcess.simulateOneLevel(level);
