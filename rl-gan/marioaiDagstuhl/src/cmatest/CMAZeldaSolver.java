...
public class CMAZeldaSolver {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<loops; i++){
            ...
            CMAZeldaSolver solver = new CMAZeldaSolver(zeldaEvalFunction, Z_SIZE, EVALS);
            FileWriter write = new FileWriter("timeline"+i+".txt", true);
            PrintWriter print_line = new PrintWriter(write);
            double[] solution = solver.run(print_line);
            print_line.close();
            System.out.println("Best solution = " + Arrays.toString(ZeldaEvalFunction.mapArrayToOne(solution)));
            bestX[i] = ZeldaEvalFunction.mapArrayToOne(solution);
            bestY[i] = solver.fitFun.valueOf(ZeldaEvalFunction.mapArrayToOne(solution));
        }
        zeldaEvalFunction.exit();
    ...
    public CMAZeldaSolver(IObjectiveFunction fitFun, int nDim, int maxEvals) {
        this.fitFun = fitFun;
        this.nDim = nDim;
